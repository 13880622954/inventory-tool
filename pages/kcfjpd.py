...
st.title("📦 盘存分析表生成器")
st.markdown("上传 IB00库存表、库位表、库龄报表和模板文件，系统将自动生成各仓库的盘存分析表（成品/赠品），并添加“是否符合”校验列。")
...
# ========== 辅助函数 ==========
def clean_str(val):
    """清理字符串，将各种类型转为字符串并去除空格，避免浮点导致endswith错误"""
    if pd.isna(val):
        return ''
    s = str(val).strip()
    # 如果字符串看起来像整数，去掉小数点
...
def get_warehouse_desc(storage_code, physical_dict, gift_dict):
    """根据库位末位判断使用哪个字典匹配仓库描述"""
...
# ========== 核心处理函数 ==========
def process_files(ib00_bytes, location_bytes, age_bytes, template_bytes):
    """处理四个文件，返回 {文件名: 字节} 字典"""
    # 读取文件
    df_ib00 = pd.read_excel(io.BytesIO(ib00_bytes))
    df_physical = pd.read_excel(io.BytesIO(location_bytes), sheet_name="实物库位表")
    try:
        df_gift = pd.read_excel(io.BytesIO(location_bytes), sheet_name="赠品库位表")
    except:
        df_gift = pd.DataFrame()
    df_age = pd.read_excel(io.BytesIO(age_bytes), sheet_name=0)

    # ---------- 1. 匹配仓库描述 ----------
    # 关键修复：强制转换存储位置为字符串
    df_ib00["存储位置"] = df_ib00["存储位置"].astype(str).apply(clean_str)
    df_physical["库位代码"] = df_physical["库位代码"].astype(str).apply(clean_str)
    df_physical["仓库描述"] = df_physical["仓库描述"].astype(str).apply(clean_str)
    if not df_gift.empty:
        df_gift["库位代码"] = df_gift["库位代码"].astype(str).apply(clean_str)
        df_gift["仓库描述"] = df_gift["仓库描述"].astype(str).apply(clean_str)

    physical_dict = dict(zip(df_physical["库位代码"], df_physical["仓库描述"]))
    gift_dict = dict(zip(df_gift["库位代码"], df_gift["仓库描述"])) if not df_gift.empty else {}

    df_ib00["仓库描述"] = df_ib00["存储位置"].apply(
        lambda x: get_warehouse_desc(x, physical_dict, gift_dict)
    )
    ...
    # 清洗库龄数据中的库位
    df_age["库位"] = df_age["库位"].astype(str).apply(clean_str)
    ...
    # 计算 ERP账面数量 = 非限制使用的库存 + 冻结库存
    if "非限制使用的库存" not in df_merged.columns:
        raise KeyError("IB00库存表中缺少“非限制使用的库存”列")
    if "冻结库存" not in df_merged.columns:
        raise KeyError("IB00库存表中缺少“冻结库存”列")
    df_merged["非限制使用的库存"] = df_merged["非限制使用的库存"].apply(clean_float)
    df_merged["冻结库存"] = df_merged["冻结库存"].apply(clean_float)
    df_merged["ERP账面数量"] = df_merged["非限制使用的库存"] + df_merged["冻结库存"]
    ...
    for warehouse in warehouses:
        ...
        if df_warehouse.empty:
            continue
        df_warehouse["存储位置_str"] = df_warehouse["存储位置"].astype(str).apply(clean_str)
    ...
    # ---------- 5. 生成汇总表 ----------
    df_merged["存储位置"] = df_merged["存储位置"].astype(str).apply(clean_str)
    ...
    # 确保成品和赠品列存在
    if "成品" not in summary_pivot.columns:
        summary_pivot["成品"] = 0
    if "赠品" not in summary_pivot.columns:
        summary_pivot["赠品"] = 0
...
with st.expander("📖 使用说明", expanded=False):
    ...
    | 文件 | 必需列/表 |
    |------|-----------|
    | **IB00库存表** | `存储位置`、`物料代码`、`工厂`、`单位`、`非限制使用的库存`、`冻结库存` 等 |
    | **库位表** | 工作表 `实物库位表` 和 `赠品库位表`，均包含 `库位代码`、`仓库描述` |
    | **库龄报表** | 包含 `物料编码`、`工厂`、`库位`、`批号`、`数量`、`库龄` |
    | **模板文件** | 工作表 `成品` 和 `赠品`，表头包含 `工厂`、`库位`、`库位名称`、`物料代码`、`物料描述`、`产品等级`、`单位`、`ERP账面数量`、`ERP账面金额`、`入库未记数`、`出库未记数`、`调整后数量`、`实盘数量`、`盘盈（+）盘亏（-）数量` 及库龄各列 |
    """)

col1, col2 = st.columns(2)
with col1:
    ib00_file = st.file_uploader("1. 上传 IB00 库存表", type=['xlsx', 'xls', 'csv'], key="ib00")
    location_file = st.file_uploader("2. 上传 库位表", type=['xlsx', 'xls', 'csv'], key="location")
with col2:
    age_file = st.file_uploader("3. 上传 库龄报表", type=['xlsx', 'xls', 'csv'], key="age")
    template_file = st.file_uploader("4. 上传 模板文件", type=['xlsx', 'xls', 'csv'], key="template")
...
if st.button("🚀 生成盘存分析表", type="primary", use_container_width=True):
    if not all([ib00_file, location_file, age_file, template_file]):
        ...
        with st.spinner("⏳ 正在处理数据，请稍候..."):
            try:
                ib00_bytes = ib00_file.read()
                location_bytes = location_file.read()
                age_bytes = age_file.read()
                template_bytes = template_file.read()

                output_files = process_files(ib00_bytes, location_bytes, age_bytes, template_bytes)
...
if st.session_state.processed and st.session_state.output_files:
    ...
    for name, data in output_files.items():
        if "汇总表" in name:
            try:
                df_summary = pd.read_excel(io.BytesIO(data))
                st.subheader("📊 汇总表预览")
...
if st.session_state.error:
    st.error(f"⚠️ 上次处理出错：{st.session_state.error}")